...
public class DelieveredOrderUtils extends CompletedOrderUtils {
    String path;
    private DatabaseReference mRef;
    private ChildEventListener mChildEventListener;
    private static int totalDelieverdOrder = 0;
    ...
    public DelieveredOrderUtils(){}

    public DelieveredOrderUtils(String email){
        path = "Users/"+email+"/DelieveredOrders";
        FirebaseDatabase.getInstance().getReference(path).keepSynced(true);
        mRef = FirebaseDatabase.getInstance().getReference(path);
        //totalDelieverdOrder = 0;
    }
    ...
    public int getCount(){
        mRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                totalDelieverdOrder = (int) snapshot.getChildrenCount();
            }
